...
model_buy = pickle.load(open("model_buy.sav", "rb"))
model_sell = pickle.load(open("model_sell.sav", "rb"))
...
def get_predict(collection, key):
    conn = MongoClient('mongodb', 27017)
    db=conn.btc
    res = []
    try:
        ...
        cur=db['realtime'].find().sort('timestamp',pymongo.DESCENDING)
        conn.close()
        for c in cur:
            res.append(float(c[key]))
        while len(res) < 10:
    ...
    if key == "buy_price":
        return model_buy.predict(res.reshape((1, -1)))[0]
    else:
        return model_sell.predict(res.reshape((1, -1)))[0]
#######################################################################################
...
init_pre_buy = get_predict('average','buy_price')
init_pre_sell = get_predict('average','sell_price')
...
source_buy_pre = ColumnDataSource(data=dict(x=[pd.to_datetime(datetime.datetime.now().replace(tzinfo = timezone.utc))], 
                                            y=[init_pre_buy]))
# line figure, the data of x axis come from key x of dict 'source_buy'
...
source_sell_pre = ColumnDataSource(data=dict(x=[pd.to_datetime(datetime.datetime.now().replace(tzinfo = timezone.utc))], 
                                         y=[init_pre_sell]))
# line figure, the data of x axis come from key x of dict 'source_sell'
...
def update():
    ...
    if cnt % 4 == 0:
        ...
        pre_buy = get_predict('average','buy_price')
        new_data_buy_pre = dict(x=[pd.to_datetime(datetime.datetime.now().replace(tzinfo = timezone.utc))],
                                y=[pre_buy]) 
        source_buy_pre.stream(new_data=new_data_buy_pre, rollover=200)
        pre_sell = get_predict('average','sell_price')
        new_data_sell_pre = dict(x=[pd.to_datetime(datetime.datetime.now().replace(tzinfo = timezone.utc))],
                                y=[pre_sell]) 
        source_sell_pre.stream(new_data=new_data_sell_pre, rollover=200)
